...
parser.add_argument('--saveDir', default='results/VAE/', type=str, metavar='DIR',
...
def train(epoch):
    ...
    for batch_idx, batch_data in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        logits, embeds, mu, logvar = model(data)
        loss_recon = F.binary_cross_entropy(logits, data.view(-1, 784), size_average=False)
        loss_kldiv = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
